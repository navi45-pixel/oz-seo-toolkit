...
async function runAudit(input) {
  ...
  // Cache headers: judge a representative STATIC asset (as Lighthouse does),
  // not the HTML document — HTML must stay revalidating to pick up deploys.
  const assetCache = await fetchT(`${origin}/favicon.ico`, { method: 'GET' }, 8000)
    .then((r) => r.headers.get('cache-control') || '')
    .catch(() => '');
  const cacheCtl = assetCache || headers['cache-control'] || '';
  const cacheOk = /max-age=\s*([0-9]+)/.test(cacheCtl) && parseInt(cacheCtl.match(/max-age=\s*([0-9]+)/)[1], 10) >= 86400;
  add('performance', 'Cache headers', cacheOk ? 'pass' : 'warn',
    cacheCtl ? `Cache-Control on static assets: ${cacheCtl}` : 'No Cache-Control header found.',
    cacheOk ? '' : 'Set long max-age for static assets (images, CSS, JS).');
  ...
  // AU formats: +61 international, landline with optional parenthesised area
  // code — "(03) 9780 9998" — mobile, 1300/1800, and 6-digit smart numbers —
  // "13 15 46" / "131 546". Lookarounds stop mid-number false matches.
  const phoneRegex = /(\+61[\s\-]?\(?0?\)?[\s\-]?(?:\d[\s\-]?\d{4}[\s\-]?\d{4}|\d{3}[\s\-]?\d{3}[\s\-]?\d{3})(?!\d))|((?<!\d)\(?0[2-8]\)?[\s\-]?\d{4}[\s\-]?\d{4}(?!\d))|((?<!\d)04[\s\-]?\d{2}[\s\-]?\d{3}[\s\-]?\d{3}(?!\d))|((?<!\d)1300[\s\-]?\d{3}[\s\-]?\d{3}(?!\d))|((?<!\d)1800[\s\-]?\d{3}[\s\-]?\d{3}(?!\d))|((?<!\d)13[\s\-]?\d{2}[\s\-]?\d{2}(?!\d))|((?<!\d)131[\s\-]?\d{3}(?!\d))/;
  const phoneMatch = bodyText.match(phoneRegex);
  // Every tel: anchor matters — sites often put a short hotline first and the
  // real number second, so scanning only .first() misses the number.
  const telHrefs = $('a[href^="tel:"]').map((i, el) => $(el).attr('href') || '').get();
  const telMatch = telHrefs.join(' ').replace(/tel:/g, ' ').match(phoneRegex);
  const telDigits = (telHrefs.map((h) => (h.match(/\+?[0-9]{7,15}/) || [null])[0]).find(Boolean)) || null;
  const schemaPhone = (html.match(/"telephone"\s*:\s*"([^"]+)"/i) || [null, null])[1];
  const anyPhone = phoneMatch ? phoneMatch[0].trim() : (telMatch && telMatch[0].trim()) || telDigits || schemaPhone;
  ...
  // ---------- 15. AI Readiness (ported from SEOMonster ai_citation_readiness) ----------
  // Tier-A evidence-backed signals from GEO paper (KDD 2024, arXiv:2311.09735).
  // Render-blindness: AI crawlers (GPTBot, ClaudeBot, PerplexityBot) don't run JS,
  // so a client-rendered SPA is blank to them — highest-value binary check.
  {
    const scripts = $('script[src]').length;
    const inlineScripts = $('script:not([src])').length;
    const totalScripts = scripts + inlineScripts;
    const renderedBlind = wordCount < 200 && totalScripts >= 3;
    add('ai-readiness', 'Render-blindness check', renderedBlind ? 'fail' : 'pass',
      renderedBlind
        ? 'RENDER-BLIND: little server-rendered text with many scripts — AI crawlers (GPTBot, ClaudeBot, PerplexityBot) do NOT execute JavaScript, so this page may be near-empty to them.'
        : `Page has ${wordCount} server-rendered words and ${totalScripts} scripts — should be extractable by AI crawlers.`,
      renderedBlind ? 'Server-render or pre-render the main content so AI crawlers can read it.' : '');

    // Extractable content: heading hierarchy (H2+ structure)
    const h2plus = headings.filter((h) => ['h2','h3','h4','h5','h6'].includes(h.tag)).length;
    const extractable = renderedBlind ? 0 : Math.min(1, 0.6 + 0.4 * (h2plus / 3));
    add('ai-readiness', 'Structured headings (extractability)', h2plus >= 3 ? 'pass' : h2plus >= 1 ? 'warn' : 'fail',
      `${h2plus} H2+ headings found — structured content is easier for AI engines to extract and cite.`,
      h2plus < 3 ? 'Add H2/H3 subheadings to structure your content into extractable chunks.' : '');

    // Statistics density: numbers in text (GEO paper: stats boost AI citation)
    const numbers = (bodyText.match(/\b\d+(?:[.,]\d+)*(?:%|k|m|b)?\b/gi) || []);
    const statsPer100w = wordCount ? (numbers.length / (wordCount / 100)) : 0;
    const statsScore = Math.min(1, statsPer100w / 2);
    add('ai-readiness', 'Statistics in content', statsScore >= 0.4 ? 'pass' : numbers.length > 0 ? 'warn' : 'fail',
      `${numbers.length} numeric values found (~${statsPer100w.toFixed(1)} per 100 words) — statistics boost AI citation likelihood.`,
      statsScore < 0.4 ? 'Include concrete numbers, percentages, or data points in your content.' : '');

    // Quotations: blockquotes + inline quotes
    const blockquotes = $('blockquote').length;
    const inlineQuotes = (bodyText.match(/"[^"]{10,}"/g) || []).length;
    const quotesScore = Math.min(1, (blockquotes + inlineQuotes) / 3);
    add('ai-readiness', 'Quotations & cited sources', quotesScore >= 0.3 ? 'pass' : 'warn',
      `${blockquotes} blockquotes, ${inlineQuotes} inline quotes — quotations increase citation potential.`,
      quotesScore < 0.3 ? 'Add blockquotes with expert opinions or customer testimonials.' : '');
    ...
    const citedScore = Math.min(1, outboundLinks / 5);
    add('ai-readiness', 'Outbound citations (linked sources)', citedScore >= 0.4 ? 'pass' : outboundLinks > 0 ? 'warn' : 'info',
      `${outboundLinks} outbound links — citing sources signals authority to AI engines.`,
      citedScore < 0.4 ? 'Link to authoritative sources (government, academic, industry) to boost credibility.' : '');
    ...
    const topRatio = contentWords.length ? topWord[1] / contentWords.length : 0;
    const stuffed = topRatio > 0.05;
    add('ai-readiness', 'Keyword stuffing check', !stuffed ? 'pass' : 'warn',
      stuffed
        ? `Top term "${topWord[0]}" appears ${(topRatio * 100).toFixed(1)}% of content words — may be keyword-stuffed (GEO paper: stuffing HURTS AI visibility).`
        : `Top term "${topWord[0]}" at ${(topRatio * 100).toFixed(1)}% — natural keyword distribution.`,
      stuffed ? 'Reduce repetition of the top keyword — aim for natural, varied language.' : '');

    // Schema.org / FAQ (reported but NOT scored — 2026 evidence: neutral-to-negative)
    const faqTypes = schemaTypes.filter((t) => /faqpage|qapage/i.test(t));
    add('ai-readiness', 'Schema.org / FAQ (informational)', faqTypes.length ? 'info' : 'info',
      faqTypes.length
        ? `FAQPage schema detected — reported for awareness. Note: Google retired FAQ rich results May 2026; keep for AI extraction only.`
        : 'No FAQPage schema — this is informational only (not scored as an AI citation driver in 2026 evidence).',
      '');

    // llms.txt (if already fetched)
    add('ai-readiness', 'llms.txt presence', llms && llms.status === 200 ? 'pass' : 'info',
      llms && llms.status === 200
        ? 'llms.txt found — emerging standard that helps LLMs understand your site structure.'
        : 'No llms.txt file — optional but recommended for AI visibility.',
      llms && llms.status === 200 ? '' : 'Consider adding /llms.txt summarising your business and key pages.');
  }

  // ---------- Scoring ----------
  const GROUPS = {
    seo: { label: 'On-Page SEO', weight: 18 },
    geo: { label: 'GEO / AI SEO', weight: 14 },
    'ai-readiness': { label: 'AI Readiness', weight: 12 },
    technical: { label: 'Technical & Health', weight: 14 },
    performance: { label: 'Speed & Performance', weight: 10 },
    security: { label: 'Security & Trust', weight: 10 },
    mobile: { label: 'Mobile', weight: 6 },
    content: { label: 'Content Quality', weight: 8 },
    local: { label: 'Local SEO (Australia)', weight: 8 },
  };
  const groupScores = {};
