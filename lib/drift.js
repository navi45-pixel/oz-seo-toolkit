 ...
 */
async function runSelfDrift({ origin }) {
  ...
  for (const [route, file] of PAGES) {
    ...
    try {
      const live = await fetchText(origin + route, SELF_TIMEOUT_MS);
  ...
  try {
    const live = await fetchText(`${origin}/definitely-not-a-page`, SELF_TIMEOUT_MS);
  ...
  try {
    const live = await fetchText(`${origin}/robots.txt`, SELF_TIMEOUT_MS);
  ...
  try {
    const live = await fetchText(`${origin}/sitemap.xml`, SELF_TIMEOUT_MS);
  ...
  try {
    const live = await fetchText(origin + '/', SELF_TIMEOUT_MS);
    const liveHeaders = {};
